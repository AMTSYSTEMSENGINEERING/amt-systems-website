...
function AvisSlider() {
  ...
  const user = [
    {
      media: (
        <a href="#">
          <img className="rounded-md" src="/twitter.png" alt="media" />
        </a>
      ),
      name: "Fa",
      pseudo: "@victofetanmi",
  ...
  return (
    <div className="bg-blue-800 p-7 avis-slider">
      ...
      <div className="w-3/4 m-auto">
        {!seeAll ? (
          <Slider {...settings} className="display-flex">
            {user.map((u, index) => (
              <div key={index} className="bg-white p-4 m-2 rounded-lg shadow-lg  w-auto max-w-sm break-words">
                <div className="flex items-center gap-3 mb-2">
                  <img src={u.avatar} alt={u.name} className="rounded-full w-10" />
                  <div className="w-full max-w-sm break-words">
                    <h3 className="font-semibold w-full max-w-full breack-words text-sm">
                      {u.name}
                    </h3>
                    <p className="avis-pseudo text-sm text-gray-500 breack-words">
                      {u.pseudo}
                    </p>
                  </div>
